# Claude Prompt (V2) — Build Exact 5-Minute PPTX for Video
...
2) If information is missing, keep an explicit placeholder in square brackets, e.g., [TEAM NAME], [MEMBER 1].
3) If sources conflict, do NOT guess. Add a visible "CONFIRM" placeholder with both values.
...
CONFLICTS YOU MUST HANDLE SAFELY
- Audio chunk length appears inconsistent across artifacts:
  - report/presentation text says 3 seconds,
  - notebook config includes MAX_AUDIO_SEC=4.
  Use placeholder text: [CONFIRM CHUNK LENGTH: 3s or 4s].
- Epoch narrative can conflict:
  - report language references a 50-epoch plan,
  - logged artifact currently contains 46 history points ending at finetune epoch 15.
  Present this carefully as "configured/planned" vs "currently logged run".
...
1) Title slide
- Project title
- Team name [placeholder if missing]
- Member names [placeholders if missing]
- Course/instructor [placeholders if missing]
...
- Use only metrics available from train_meta and loss_curves
- One chart slide showing total/sv/df trends
- What worked, what is pending, why no benchmark metrics yet

7) Limitations + validation status
- Clearly label EER/min-DCF as planned if not executed
...
3) Then provide a final "Fact-Check Table" with 3 columns:
   - Claim in slides | Source file | Status (Verified / Placeholder / Planned)
4) End with a "Missing Info To Fill" checklist containing only placeholders that still need user input.
